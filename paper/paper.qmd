...
```{r}
#| label: applications
#| fig-cap: New and total applications comparison
#| echo: false

# Load the required libraries
library(dplyr)

# First, ensure that the Quarter/Year column is read correctly
data$Quarter_Year <- as.character(analysis_data$Quarter_Year)

# Filter data for specific quarters
specific_quarters <- data %>%
  filter(Quarter_Year %in% c("Q1 2019", "Q1 2020", "Q1 2022", "Q1 2023")) %>%
  select(Quarter_Year, New_reactivated = New_reactivated, Total_active_waiting_list = Total_active_waiting_list)
...
# Create a line plot
ggplot(plot_data, aes(x = quarter_year)) +
  geom_line(aes(y = Inactive_Cancelled, color = "Inactive/Cancelled"), size = 1.2) +
  geom_line(aes(y = Total_Active_Waiting_List, color = "Total Active Waiting List"), linetype = "dashed", size = 1.2) +
  scale_color_manual(values = c("blue", "red")) +
